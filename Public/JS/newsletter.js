...
if(document.getElementById("newsletterForm")) {
    document.getElementById("newsletterForm").addEventListener("submit", function (e) {
        ...
        if (form.namesurname.value === "" || form.email.value === "") {
            showPopup("error", "Please fill in all fields.");
            return;
        }

        var csrfToken = document.getElementById("csrf_token-newsletter-form").value;
        var cfTurnStile = document.getElementById("cf-token-newsletter-form").value;
        ...
        formData.append("cf-token", cfTurnStile);

        var xhr = new XMLHttpRequest();
        xhr.open("POST", "/?/control/form/post/newsletterForm", true);
        xhr.send(formData);
        xhr.onload = function () {
            if (xhr.status === 200) {
