...
function q($deger)
{
    ...
    elseif(strpos($sayfaadres, $deger)!==false)
    {
        $ayiklabaslasayi=stripos($sayfaadres,$deger."=");
        $yenimetin=substr($sayfaadres,$ayiklabaslasayi,strlen($sayfaadres)-$ayiklabaslasayi);
        $yenimetin=str_replace($deger."=", "", $yenimetin);
        $ayiklabitir="&";
        if(strpos($yenimetin,$ayiklabitir)!==false)
        {
            $ayiklabitirsayi=stripos($yenimetin,$ayiklabitir);
            $yenimetin =substr($yenimetin, 0,$ayiklabitirsayi);
        }
        unset($ayiklabaslasayi,$ayiklabitir);
        return $yenimetin;
    }
}

$guvenlikoturum='_CAPTCHA';
if(!BosMu(q("guvenlikoturum"))){$guvenlikoturum=q("guvenlikoturum");}


function simple_php_captcha($config = array())
{
    global $guvenlikoturum;
    ...
    */
    $image_src='/1.php?guvenlikoturum='.$guvenlikoturum.'&'.$guvenlikoturum.'&amp;t=' . urlencode(microtime());
    $_SESSION[$guvenlikoturum]['config'] = serialize($captcha_config);
    $_SESSION[$guvenlikoturum]['code'] = $captcha_config['code'];
    $_SESSION[$guvenlikoturum]['image_src'] = $image_src;
...
// Draw the image
if( isset($_GET[$guvenlikoturum]) )
{
    $captcha_config = unserialize($_SESSION[$guvenlikoturum]['config']);
