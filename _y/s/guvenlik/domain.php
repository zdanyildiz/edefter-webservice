...
require_once("$path/_y/s/guvenlik/log/index.php");
require_once("$path/s/f/index.php");
?>
<?php
	//form değişkenler
	$f_domain="";
	...
	if (isset($_POST['olustur'])) 
	{
		if($_POST['olustur']==1)
		{
			$f_domain=$_POST['domain'];
			if(!empty($f_domain))
			{
				$domainsayfa = $path.'/s/v/domain/index.php';
			...
			if($formhata==0)
			{
				$f_dbname=$_POST['dbname'];
				$f_dbuser=$_POST['dbuser'];
				$f_dbpass=$_POST['dbpass'];
				if(!empty($f_dbname) && !empty($f_dbuser) && !empty($f_dbpass))
				{
					// Check connection
					//$data = new mysqli("94.73.131.138", $f_dbuser, $f_dbpass,$f_dbname);
					mysqli_report(MYSQLI_REPORT_STRICT);
					try 
					{
						$connection = new mysqli('94.73.131.138', $f_dbuser, $f_dbpass, $f_dbname);

						$sqlsayfa = $path.'/s/v/sql/index.php';
						file_put_contents($sqlsayfa,'<?php'.PHP_EOL.'$servername = "94.73.131.138";'.PHP_EOL.'$username = "'.$f_dbuser.'";'.PHP_EOL.'$password = "'.$f_dbpass.'";'.PHP_EOL.'$database = "'.$f_dbname.'";'.PHP_EOL.'?>');
						unset($sqlsayfa);
					} 
					catch (Exception $e ) 
					{
			...
			if($formhata==0)
			{
				$f_adminname=$_POST['adminname'];
				$f_adminmail=$_POST['adminmail'];
				$f_admingsm=$_POST['admingsm'];
				$f_adminauth=$_POST['adminauth'];
				if(!empty($f_adminname) && !empty($f_adminmail))
				{
					...
					INSERT INTO yoneticiler 
						(
						YoneticiEklenmeTarih, 
						YoneticiGuncellenmeTarih,
						YoneticiYetki,
						YoneticiAdSoyad,
						YoneticiEposta,
						YoneticiCepTelefon,
					...
					VALUES 
						...
						'".$f_adminauth."',
						'".$f_adminname."',
						'". urlencode(sifrele($f_adminmail,$anahtarkod)) ."',
					...
					{
						$anahtarsayfa = $path.'/s/v/anahtar/index.php';
						file_put_contents($anahtarsayfa,'<?php $anahtarkod="'.$anahtarkod.'";?>');
						$mailIcerik="<p>Sayin $f_adminname Sistem Yazılım Merkezi dünyasına hoş geldiniz.</p><p>$f_domain sitenizin bazı verileri, kullanıcı bilgileri vs. özel bir anahtar kod ile şifrelenmektedir. Siteniz çalıştığı sürece bu anahtar koduna ihtiyacınız olmayacaktrı. Fakat ilerde yeni kurulum yapmanız ya da şifreli verileri çözmeniz gerekirse lütfen aşağıdaki anahtar kodunu webmaster/sistem yöneticisi yetkilisine bildiriniz.</p><p>Anahtar Kod: $anahtarkod </p><p>Zafer DANYILDIZ<br>Sistem Medya Bilgi İşlem Müdürü<br>
						5326270247 | zd@sistemyazilimmerkezi.com</p>";
						MailGonder($yoneticieposta,"$f_domain Anahtar Kod Bilgilendirmesi",$mailIcerik);
						unset($anahtarsayfa,$anahtarkod);
					}
					else
					{
						hatalogisle("YöneticiEkleDomain",$data->error);
						$formhata=1;
