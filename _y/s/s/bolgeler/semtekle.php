...
$f_semtad="";
//düzenle
$sayfabaslik="Semt Düzenle";
$formbaslik="Semt Ekle";
$butonisim="KAYDET";

if(S(f("semtekle"))==1 && !BosMu(f("AreaName")) && S(f("CountyID"))!=0)
{
	$f_semtad=f("AreaName");
    $f_ilceid=f("CountyID");
	Veri(true);
	...
	if(!Dogrula("yersemt","CountyID='".$f_ilceid."' and AreaName='".$f_semtad."'")&&S(f("semtduzenle"))==0)
	{
        $formhata=0;
        $formhataaciklama="Yeni Semt Eklendi";
        ekle("AreaName,CountyID",$f_semtad."|*_".$f_ilceid,"yersemt",0);
	}
    elseif(Dogrula("yersemt","AreaID='".S($f_semtid)."'") && S(f("semtduzenle"))==1)
    {
        $formhata=0;
        $formhataaciklama="Semt Güncellendi";
        guncelle("AreaName",$f_semtad,"yersemt","AreaID='".$f_semtid."'",0);
    }
	else
    {
        $formhata=1;
        $formhataaciklama="Bu ilçeye kayıtlı aynı isimde semt var";
    }
	/**
     * SemtId büyüktür 2469 düzenlemeye izin ver
     */
}

if(S(q("semtid"))>2469)
{
    $semt_bilgi=coksatir("SELECT CityID,yerilce.CountyID,AreaName FROM yersemt inner join yerilce ON yerilce.CountyID=yersemt.CountyID WHERE AreaID='".q("semtid")."'");
    if(!BosMu($semt_bilgi))
    {
        $f_sehirid=$semt_bilgi["CityID"];
        $f_ilceid=$semt_bilgi["CountyID"];
        $f_semtad=$semt_bilgi["AreaName"];
    }
}
?>
<!DOCTYPE html>
<html lang="tr">
	...
	<body class="menubar-hoverable header-fixed ">
		...
		<div id="base">
			...
			<div id="content">
				<section>
					...
					<?php if($formhata==0 && S(f("semtekle"))==1){ ?>
					<div class="alert alert-success" role="alert">
						<?=$formhataaciklama?>
					</div>
					<?php }elseif($formhata==1 && S(f("semtekle"))==1) { ?>
							...
							<div class="col-md-12">
								<div class="card">
									...
									<form class="form form-validation form-validate" role="form" method="post">
										<input type="hidden" name="semtekle" value="1">
                                        <?php if(S(q("semtid"))!=0){?><input type="hidden" name="semtduzenle" value="1"><input type="hidden" name="semtid" value="<?=q("semtid")?>"><?php }?>
                                                                    ...
                                                                    if($data->query($ilce_s))
                                                                    {
                                                                        ...
                                                                        if($ilce_v->num_rows>0)
                                                                        {
                                                                            while($ilce_t=$ilce_v->fetch_assoc())
                                                                            {
                                                                                ...
                                                                                ?>
                                                                                <option value="<?=$l_ilceid?>"><?=$l_ilcead?> </option>
                                                                                <?php
                                                                            }unset($ilce_t);
                                                                        }unset($ilce_v);
                                                                    }else{die($data->error);}
                                                    ...
                                                    <div class="row">
														<div class="col-md-4">
															<div class="form-group floating-label">
																<input type="text" class="form-control" id="AreaName" name="AreaName" value="<?=$f_semtad?>" required="" aria-required="true" aria-invalid="true">
																<label for="AreaName">Semt Adı</label>
															</div>
														</div>
                                                    </div>
													</div><!--end .row -->
		...
		<script>
			$("#semteklephp").addClass("active");
            $("#CityID").on("change",function()
            {
                $sehirid=$(this).val();
                window.location.href="/_y/s/s/bolgeler/semtekle.php?sehirid="+$sehirid;
            });
		</script>
		<!-- END JAVASCRIPT -->
	</body>
