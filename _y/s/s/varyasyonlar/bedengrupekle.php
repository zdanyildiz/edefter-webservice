...
//düzenle
$sayfabaslik="Beden Gruplar";
$formbaslik="Ürün Beden Grupları";
$butonisim="EKLE";

$f_urunbedengrupid=f("urunbedengrupid");
$f_urunbedengrupad=f("urunbedengrupad");

if(S(f("urunbedengrupekle"))==1 && !BosMu($f_urunbedengrupad))
{
	$sutunlar="urunbedengrupad,urunbedengrupsil";
	$degerler=$f_urunbedengrupad."|*_"."0";
	$tablo="urunbedengrup";

	if(dogrula("urunbedengrup","urunbedengrupad='". $f_urunbedengrupad ."' and urunbedengrupsil='0' ") && S($f_urunbedengrupid)==0)
	{
		$formhata=1;
		$formhataaciklama="DİKKAT: Bu isimde ( $f_urunbedengrupad ) zaten grup var. Lütfen ilgili kaydı düzenleyiniz.<br><br>
		<a href='/_y/s/s/varyasyonlar/bedengrupliste.php'> > Beden Grup Listesine git <</a><br>";
	}
	if($formhata==0)
	{
		if(S($f_urunbedengrupid)==0)
		{
			...
			$f_urunbedengrupid = teksatir(" Select urunbedengrupid from urunbedengrup Where benzersizid='". $f_benzersizid ."'","urunbedengrupid");
		}
		else
		{
			guncelle($sutunlar,$degerler,$tablo," urunbedengrupid='". $f_urunbedengrupid ."' ",35);
		}
	}
}
if(S(q("urunbedengrupid"))!=0)
{
	if(dogrula("urunbedengrup","urunbedengrupid='". q("urunbedengrupid") ."'"))
	{
		$butonisim="GÜNCELLE";
		$f_urunbedengrupid=q("urunbedengrupid");
		$f_urunbedengrupad=teksatir("Select urunbedengrupad From urunbedengrup Where urunbedengrupid='". q("urunbedengrupid") ."'","urunbedengrupad");
	}
}
?>
<!DOCTYPE html>
<html lang="tr">
	...
	<body class="menubar-hoverable header-fixed ">
				...
				<section>
					<div class="section-header">
						<ol class="breadcrumb">
							...
							<li class="active"><a href="/_y/s/s/varyasyonlar/bedengrupliste.php" class="btn ink-reaction btn-raised btn-primary">Beden Grup Liste</a></li>
						</ol>
					</div>
							...
							<div class="col-md-12">
								<div class="card">
									...
									<form name="formanaliz" class="form form-validation form-validate" role="form" method="post">
										<input type="hidden" name="urunbedengrupekle" value="1">
										<input type="hidden" name="urunbedengrupid" value="<?=$f_urunbedengrupid?>">
													...
													<div class="row">															
														<div class="col-sm-6">
															...
															<input 
																type="text" 
																class="form-control" 
																name="urunbedengrupad" 
																id="urunbedengrupad" 
																value="<?=$f_urunbedengrupad?>"
																placeholder="Beden Grup Adını Yazın" required aria-required="true" >
																<label for="urunbedengrupad">Beden Grup Adını Yazın</label>
															</div>
														</div>
													</div>
												</div>
		...
		<script>
			$("#bedengrupeklephp").addClass("active");
		</script>
		<!-- END JAVASCRIPT -->
	</body>
</html>