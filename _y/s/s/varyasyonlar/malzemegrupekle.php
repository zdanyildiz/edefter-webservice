...
//düzenle
$sayfabaslik="malzeme Gruplar";
$formbaslik="Ürün Malzeme Grupları";
$butonisim="EKLE";

$f_urunmalzemegrupid=f("urunmalzemegrupid");
$f_urunmalzemegrupad=f("urunmalzemegrupad");

if(S(f("urunmalzemegrupekle"))==1 && !BosMu($f_urunmalzemegrupad))
{
	$sutunlar="urunmalzemegrupad,urunmalzemegrupsil";
	$degerler=$f_urunmalzemegrupad."|*_"."0";
	$tablo="urunmalzemegrup";

	if(dogrula("urunmalzemegrup","urunmalzemegrupad='". $f_urunmalzemegrupad ."' and urunmalzemegrupsil='0'") && S($f_urunmalzemegrupid)==0)
	{
		$formhata=1;
		$formhataaciklama="DİKKAT: Bu isimde ( $f_urunmalzemegrupad ) zaten grup var. Lütfen ilgili kaydı düzenleyiniz.<br><br>
		<a href='/_y/s/s/varyasyonlar/malzemegrupliste.php'> > malzeme Grup Listesine git <</a><br>";
	}
	if($formhata==0)
	{
		if(S($f_urunmalzemegrupid)==0)
		{
			...
			$f_urunmalzemegrupid = teksatir(" Select urunmalzemegrupid from urunmalzemegrup Where benzersizid='". $f_benzersizid ."'","urunmalzemegrupid");
		}
		else
		{
			guncelle($sutunlar,$degerler,$tablo," urunmalzemegrupid='". $f_urunmalzemegrupid ."' ",35);
		}
	}
}
if(S(q("urunmalzemegrupid"))!=0)
{
	if(dogrula("urunmalzemegrup","urunmalzemegrupid='". q("urunmalzemegrupid") ."'"))
	{
		$butonisim="GÜNCELLE";
		$f_urunmalzemegrupid=q("urunmalzemegrupid");
		$f_urunmalzemegrupad=teksatir("Select urunmalzemegrupad From urunmalzemegrup Where urunmalzemegrupid='". q("urunmalzemegrupid") ."'","urunmalzemegrupad");
	}
}
?>
<!DOCTYPE html>
<html lang="tr">
	...
	<body class="menubar-hoverable header-fixed ">
				...
				<section>
					<div class="section-header">
						<ol class="breadcrumb">
							...
							<li class="active"><a href="/_y/s/s/varyasyonlar/malzemegrupliste.php" class="btn ink-reaction btn-raised btn-primary">Malzeme Grup Liste</a></li>
						</ol>
					</div>
							...
							<div class="col-md-12">
								<div class="card">
									...
									<form name="formanaliz" class="form form-validation form-validate" role="form" method="post">
										<input type="hidden" name="urunmalzemegrupekle" value="1">
										<input type="hidden" name="urunmalzemegrupid" value="<?=$f_urunmalzemegrupid?>">
													...
													<div class="row">															
														<div class="col-sm-6">
															...
															<input 
																...
																id="urunmalzemegrupad" 
																value="<?=$f_urunmalzemegrupad?>"
																placeholder="malzeme Grup Adını Yazın" required aria-required="true" >
																<label for="urunmalzemegrupad">Malzeme Grup Adını Yazın</label>
															</div>
														</div>
													</div>
												</div>
		...
		<script>
			$("#malzemegrupeklephp").addClass("active");
		</script>
		<!-- END JAVASCRIPT -->
	</body>
</html>