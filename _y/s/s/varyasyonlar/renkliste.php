...
$formhata=0;
$formhataaciklama="";
$formtablo="urunrenkgrup";

//düzenle
$sayfabaslik="Ürün Renk";
$formbaslik="Ürün Renk Liste";

Veri(true);
if(S(f("renksirala"))==1)
{
    foreach($_POST['urunrenkid'] as $urunrenkid)
    {
        $renksira =S(f("renksira".$urunrenkid));
        guncelle("urunrenksira",$renksira,"urunrenk","urunrenkid='".$urunrenkid."'",0);
    }
}
$urunrenkgrup_d=0;
$urunrenkgrup_s="
	SELECT 
		urunrenkgrupad,urunrenkad,urunrenksira,urunrenkid,urunrenkkod
	FROM 
		urunrenkgrup 
			inner join urunrenk on urunrenk.urunrenkgrupid=urunrenkgrup.urunrenkgrupid
	WHERE urunrenkgrup.urunrenkgrupsil='0' and urunrenksil='0'
	ORDER BY urunrenkgrup.urunrenkgrupid, urunrenk.urunrenkad,urunrenk.urunrenksira Asc";
if($data->query($urunrenkgrup_s)){$urunrenkgrup_v=$data->query($urunrenkgrup_s);
if($urunrenkgrup_v->num_rows>0)$urunrenkgrup_d=1;
unset($urunrenkgrup_s);}else{hatalogisle("renkliste",$data->error);}
?>
<!DOCTYPE html>
<html lang="tr">
<head>
        ...
        <section>
            <div class="section-header">
                <ol class="breadcrumb">
                    ...
                    <li class="btn ink-reaction btn-raised btn-primary disabled">Ürun Renkler</li>
                    <li class="active"><a href="/_y/s/s/varyasyonlar/renkekle.php" class="btn ink-reaction btn-raised btn-primary"><?=$sayfabaslik?> Ekle (+)</a></li>
                </ol>
            </div>
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-head style-primary">
                                ...
                                <div class="tools">
                                    <a class="btn btn-floating-action btn-default-light" href="/_y/s/s/varyasyonlar/renkekle.php"><i class="fa fa-plus"></i></a>
                                </div>
                            </div>
                        </div><!--end .card -->
                    </div><!--end .col -->
                            ...
                            <div class="card-body ">
                                <table class="table no-margin">
                                    <form class="form form-validation form-validate" role="form" method="post">
                                        <input type="hidden" name="renksirala" value="1">
                                        <thead>
                                        <tr>
                                            <th>#</th>
                                            <th>Renk Grup Ad</th>
                                        ...
                                        if($urunrenkgrup_d==1)
                                        {
                                            while ($urunrenkgrup_t=$urunrenkgrup_v->fetch_assoc())
                                            {
                                                $urunrenkid=$urunrenkgrup_t["urunrenkid"];
                                                $urunrenkad=$urunrenkgrup_t["urunrenkad"];
                                                $urunrenksira=$urunrenkgrup_t["urunrenksira"];
                                                $urunrenkgrupad=$urunrenkgrup_t["urunrenkgrupad"];
                                                $urunrenkkod=$urunrenkgrup_t["urunrenkkod"];
                                                ?>
                                                <tr id="tr<?=$urunrenkid?>">
                                                    <td style="background-color:<?=$urunrenkkod?>"><?=$urunrenkid?></td>
                                                    <td><?=$urunrenkgrupad?></td>
                                                    <td><?=$urunrenkad?></td>
                                                    <td class="col-md-3"><input type="hidden" name="urunrenkid[]" value="<?=$urunrenkid?>"><input name="renksira<?=$urunrenkid?>" type="number" value="<?=$urunrenksira?>" class="form-control"></td>
                                                    <td>
                                                        <a
                                                                href="/_y/s/s/varyasyonlar/renkekle.php?urunrenkid=<?=$urunrenkid?>"
                                        ...
                                        unset($urunrenkgrup_d,$urunrenkgrup_v,$urunrenkgrupid,$urunrenkgrupkisaad);
                                        ?>
                                        <tr>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="simpleModalLabel">Renk Sil</h4>
            </div>
            <div class="modal-body">
                <p>Renki silmek istediğinize emin misiniz?</p>
            </div>
...
<script>
    ...
    {
        $(document).on("click",'a#Renksil',function ()
        {
            $silid=$(this).data("id");
        });
        $(document).on("click",'#silbutton',function ()
        {
            $('#_islem').attr('src', "/_y/s/f/sil.php?sil=renk&id="+$silid);
        });
    });
</script>
<script>
    $("#renklistephp").addClass("active");
</script>
</body>
</html>
