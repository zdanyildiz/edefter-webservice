...
<div id="base">
                        ...
                        <form name="addAddressForm" id="addAddressForm" class="form form-validation form-validate" role="form" method="post">
                            ...
                            <div class="modal-content">
                                <div class="modal-header">
                                    ...
                                    <h4 class="modal-title">Adres Ekle/Düzenle</h4>
                                </div>
    ...
    <div class="modal fade" id="deleteAddressModal" tabindex="-1" role="dialog" aria-labelledby="deleteAddressModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="card">
                <div class="card-head card-head-sm style-danger">
                    <header class="modal-title" id="deleteAddressModalLabel">Adres Silme Onayı</header>
                    <div class="tools">
                        <div class="btn-group">
                            <a class="btn btn-icon-toggle btn-close" data-dismiss="modal" aria-hidden="true">
                                <i class="fa fa-close"></i>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">İptal</button>
                    <button type="button" class="btn btn-danger" id="deleteAddressConfirmButton">Adresi Sil</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    // Adres silme modalı için değişkenler
    var selectedAddressIdForDelete = 0;

    // Adres sil butonuna tıklandığında
    $(document).on("click", "[id='adressil']", function(){
        selectedAddressIdForDelete = $(this).data("id");
        $("#deleteAddressModal").modal("show");
    });

    // Adres silme onay butonuna tıklandığında
    $(document).on("click", "#deleteAddressConfirmButton", function(){
        if(selectedAddressIdForDelete <= 0){
            $("#alertModal .card-head").removeClass("style-success").addClass("style-danger");
            $("#alertMessage").text("Adres ID bulunamadı");
            $("#alertModal").modal("show");
            return;
        }
        ...
        if(memberID <= 0){
            $("#alertModal .card-head").removeClass("style-success").addClass("style-danger");
            $("#alertMessage").text("Üye ID bulunamadı");
            $("#alertModal").modal("show");
            return;
        }
        ...
        $.ajax({
            ...
            data: {
                ...
                addressID: selectedAddressIdForDelete,
                memberID: memberID
            },
            success: function(data){
                console.log(data);
            ...
            error: function(){
                $("#alertModal .card-head").removeClass("style-success").addClass("style-danger");
                $("#alertMessage").text("Sunucu hatası oluştu");
                $("#alertModal").modal("show");
                $("#deleteAddressModal").modal("hide");
            }
        });
