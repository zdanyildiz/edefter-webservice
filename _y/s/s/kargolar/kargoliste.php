...
//düzenle
$sayfabaslik="Urun kargo Listesi";
$formbaslik="Urun kargo Liste";

Veri(true);
$kargo_d=0;
$kargo_s="SELECT kargoid,kargoad,kargoCode FROM kargo WHERE kargosil='0' ORDER BY kargoid ASC";
if($kargo_v=$data->query($kargo_s))
{
    $kargo_v=$data->query($kargo_s);
    if($kargo_v->num_rows>0)$kargo_d=1;
}
else{hatalogisle("kargo liste",$data->error);}
unset($kargo_s);
?>
<!DOCTYPE html>
<html lang="tr">
				...
				<section>
					<div class="section-header">
						<ol class="breadcrumb">
							...
							<li class="btn ink-reaction btn-raised btn-primary disabled">Kargolar</li>
							<li class="active"><a href="/_y/s/s/kargolar/kargoekle.php" class="btn ink-reaction btn-raised btn-primary"><?=$sayfabaslik?> Ekle (+)</a></li>
						</ol>
					</div>			
								...
								<div class="card">
									<div class="card-body ">
										<table class="table no-margin">
											...
											if($kargo_d==1)
											{
												while ($kargo_t=$kargo_v->fetch_assoc()) 
												{
													$kargoid=$kargo_t["kargoid"];
													$kargoad=$kargo_t["kargoad"];
                                                    $kargoCode=$kargo_t["kargoCode"];
												?>
												<tr id="tr<?=$kargoid?>">
													<td><?=$kargoid?></td>
													<td><?=$kargoad?></td>
													...
													<td>
														<a 
															href="/_y/s/s/kargolar/kargoekle.php?kargoid=<?=$kargoid?>" 
