...
if(!empty($salesConversionCode)){
    $salesConversionCodeID = $salesConversionCode[0]['salesConversionCodeID'];
    $salesConversionCodeName = $salesConversionCode[0]['salesConversionCodeName'];
    $salesConversionCodeContent = $salesConversionCode[0]['salesConversionCodeContent'];
}
$salesConversionCodeID = $salesConversionCodeID ?? 0;
...
<html lang="tr">
	<head>
		<title>Satış Dönüşüm Kodu Ekle Pozitif Eticaret</title>
		<meta charset="utf-8">
	...
	<body class="menubar-hoverable header-fixed ">
				...
				<section>
					...
					<div class="section-body contain-lg">
						<div class="row">
							...
							<div class="col-md-12">
								<div class="card">
									<form name="salesConversionCodeForm"  id="salesConversionCodeForm" class="form form-validation form-validate" role="form" method="post">
										<div class="card-body">
											<div class="row">
												<div class="col-xs-12">
													<div class="row">
														...
														<div class="col-sm-3" style="margin-top:15px">
															<input 
																type="text" 
																class="form-control" 
																name="salesConversionCodeName"
																id="salesConversionCodeName"
																value="<?=$salesConversionCodeName?>"
																placeholder="Örn: Google-Facebook" >
															<label for="salesConversionCodeName">Dönüşüm Kod Adını Yazın</label>
														</div>
													</div>
													<div class="row">
														<div class="form-group floating-label">
															<textarea 
																name="salesConversionCodeContent"
																id="salesConversionCodeContent"
																class="form-control" 
																rows="7" 
																...
																><?=trim($salesConversionCodeContent)?></textarea>
															<label for="salesConversionCodeContent" style="margin-top:-20px">Satış Dönüşüm Kodu</label>
														</div>
													</div>
												</div>
											</div>
		...
		<script>
			$("#salesConversionCodephp").addClass("active");

            //languageID değiştiğinde
            $("#languageID").change(function(){
                var languageID = $(this).val();
                window.location.href = "/_y/s/s/ekkodlar/SalesConversionCode.php?languageID="+languageID;
            });

            //Form submit
            $("#salesConversionCodeForm").submit(function(e){
                e.preventDefault();
                var salesConversionCodeName = $("#salesConversionCodeName").val();
                var salesConversionCodeContent = $("#salesConversionCodeContent").val();
                var languageID = $("#languageID").val();
                if(salesConversionCodeName == ""){
                    $("#alertModal .card-head").removeClass("style-success").addClass("style-danger");
                    $("#alertMessage").html("Dönüşüm Kod Adını Girin");
                    $("#alertModal").modal("show");
                    return false;
                }
                if(salesConversionCodeContent == ""){
                    $("#alertModal .card-head").removeClass("style-success").addClass("style-danger");
                ...
                $.ajax({
                    ...
                    type: "POST",
                    data: {
                        action: "saveSalesConversionCode",
                        salesConversionCodeID: <?=$salesConversionCodeID?>,
                        salesConversionCodeName: salesConversionCodeName,
                        salesConversionCodeContent: salesConversionCodeContent,
