...
$formhata=0;
$formhataaciklama="";
$formtablo="kargoucret";

//düzenle
$sayfabaslik="Ürün Kargo Ücretlendirme Listesi";
$formbaslik="Ürün Kargo Ücretlendirme Listesi";

Veri(true);
$kargoucret_d=0;
$kargoucret_s="SELECT kargoucretid,kargosabitucret,kargoucretsiz,kargourunadet,kargosuresi FROM kargoucret ORDER BY kargoucretid ASC";
if($data->query($kargoucret_s))
{
    $kargoucret_v=$data->query($kargoucret_s);
    if($kargoucret_v->num_rows>0)$kargoucret_d=1;
}else{die($data->error);}
unset($kargoucret_s);
?>
<!DOCTYPE html>
<html lang="tr">
	...
	<body class="menubar-hoverable header-fixed ">
				...
				<section>
					<div class="section-header">
						<ol class="breadcrumb">
							...
							<li class="btn ink-reaction btn-raised btn-primary disabled">Kargo Ücret Listesi</li>
						<?PHP /*	<li class="active"><a href="/_y/s/s/kargolar/kargoekle.php" class="btn ink-reaction btn-raised btn-primary"><?=$sayfabaslik?> Ekle (+)</a></li> */?>
						</ol>
					</div>			
					<div class="section-body contain-lg">
						<div class="row">
							...
							<?PHP /*			<div class="tools">
											<a class="btn btn-floating-action btn-default-light" href="/_y/s/s/kargolar/kargoekle.php"><i class="fa fa-plus"></i></a>
										</div>*/ ?>
									</div>
								</div><!--end .card -->
							</div><!--end .col -->
								...
								<div class="card">
									<div class="card-body ">
										<table class="table no-margin">
											<thead>
												<tr>
                                                    <th>Kargo Sabit Ücret</th>
                                                    <th>Kargo Süresi</th>
													<th>X TL Üzeri Ücretsiz Kargo</th>
                                                 <?PHP /*   <th>X Adet Ürün Kadar Kargo Ücretsiz</th>*/ ?>
                                                    <th>İşlem</th>
												</tr>
											</thead>
											<tbody>
											...
											if($kargoucret_d==1)
											{
												while ($kargoucret_t=$kargoucret_v->fetch_assoc())
												{
													$kargoucretid=$kargoucret_t["kargoucretid"];
													$kargosabitucret=$kargoucret_t["kargosabitucret"];
                                                    $kargoucretsiz=$kargoucret_t["kargoucretsiz"];
                                                    $kargourunadet=$kargoucret_t["kargourunadet"];
                                                    $kargosuresi=$kargoucret_t["kargosuresi"];
												?>
												<tr id="tr<?=$kargoucretid?>">
													<td><?=$kargosabitucret?></td>
                                                    <td><?=$kargosuresi?></td>
													<td><?=$kargoucretsiz?></td>
                                              <?PHP /*      <td><?=$kargourunadet?></td> */ ?>
													<td>
														<a 
															href="/_y/s/s/kargoucret/kargoucretekle.php?kargoucretid=<?=$kargoucretid?>"
											...
											unset($kargo_d,$kargo_v,$kargoid,$kargokisaad);
											?>
											</tbody>
										</table>
									</div><!--end .card-body -->
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title" id="simpleModalLabel">Kargo Sil</h4>
					</div>
					<div class="modal-body">
						<p>Kargo silmek istediğinize emin misiniz?</p>
					</div>
		...
		<script>
			...
			{
				$('a#kargosil').click(function ()
				{
					$silid=$(this).data("id");
				});
				$('#silbutton').click(function ()
				{
					$('#_islem').attr('src', "/_y/s/f/sil.php?sil=kargoi&id="+$silid);
				});
			 });
		</script>
		<script>
